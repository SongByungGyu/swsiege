...
**Architecture:** Phase 1 인증 위에 (1) season 테이블 제거 + 몬스터 마스터 seed, (2) 길드/방어덱/공격기록 수동 입력 폼, (3) 부분 매칭 검색 + 공격 통계 집계 API/UI를 쌓는다. 시즌 분리는 폐기하고 모든 기록을 시간순 누적. 모든 DB 접근은 서버측 Next.js API 라우트를 거쳐 `getSupabaseAdmin()`로 service_role로 처리.
...
│   ├── 20260514000000_drop_season.sql             # NEW: season 테이블/컬럼 제거
...
## Task 3: schema migration — season 테이블/컬럼 제거

**Files:**
- Create: `supabase/migrations/20260514000000_drop_season.sql`

길드 운영 결정: 시즌별 분리 폐기, 모든 기록을 시간순으로 누적. `season` 테이블과 `defense_deck.season_id` 외래키를 제거. 같은 길드의 옛/새 방어덱이 검색에서 함께 보이지만 `captured_at` 정렬로 구분.

- [ ] **Step 1: drop migration SQL 작성**

`supabase/migrations/20260514000000_drop_season.sql`:
...
```bash
cat supabase/migrations/20260514000000_drop_season.sql | pbcopy
```

→ 대시보드 SQL Editor → 붙여넣고 Run.
→ Table Editor에서:
- `season` 테이블이 사라졌는지 확인 (4개 테이블만 남음: monster, enemy_guild, defense_deck, attack_record)
- `defense_deck` 테이블 컬럼 목록에서 `season_id`가 없는지 확인

- [ ] **Step 3: commit**

```bash
git add supabase/migrations/20260514000000_drop_season.sql
git commit -m "feat: schema migration - season 테이블 제거 (시즌 분리 폐기)"
git push
```

---

## Task 4: Supabase 타입 생성 + getSupabaseAdmin<Database>

**Files:**
- Create: `src/types/database.ts`
- Modify: `src/lib/supabase.ts`

Phase 1 final reviewer 권장사항. CRUD 코드의 타입 안전성 확보. **Task 3의 schema 변경(season drop)이 적용된 후 실행해야** 타입에 season 잔재가 없음.
...
기대: `Database` export 보임, **4개 테이블**(monster, enemy_guild, defense_deck, attack_record)의 Row/Insert/Update 타입 포함, season 타입은 없어야 함. `defense_deck` 타입에 `season_id` 필드도 없어야 함. 100~200줄 정도.
...
SWARFARM 공개 API에서 **모든 자연 등급(2★~5★) + 미각성/각성 + 2차 각성 몬스터**를 fetch해 INSERT SQL을 생성. 게임 내 점령전에서 어떤 등급이든 사용될 수 있으므로 광범위 시드가 필요. SW 게임 내 `monster_id`를 그대로 사용해야 Phase 3 SWEX 플러그인 데이터와 매칭됨.
...
기대: 2-5분 소요(전체 페이지네이션). 파일 크기 수백 KB ~ 2 MB 사이. SWARFARM의 전체 몬스터 수는 약 1500~2000마리.
...
기대: 첫 줄에 SQL 주석, 마지막에 `ON CONFLICT (id) DO NOTHING;`. **1000~2000 줄 정도** (자연 2~5★ + 각성/미각성/2차각성 합계).
...
git commit -m "feat: SWARFARM API에서 전체 몬스터(2~5★, 각성/미각성/2차각성) seed SQL 생성"
git push
```
...
1000마리 이상의 행이 들어갔는지 확인. SQL 실행에 30초~1분 정도 걸릴 수 있음(대용량 INSERT).
...
- [ ] season 테이블 drop, 몬스터 마스터 데이터(1000+) seed 적용
